...
BEGIN
	SET @QUERY = '
        ...
        SELECT ''t_testdata_groupShared1.jpg'', ''t_testdata_groupShared1.jpg'', U.Id FROM ['+@dbSchema+'].[Users] U
        WHERE U.Username = ''t_testdata_CustomUser1''

        INSERT INTO ['+@dbSchema+'].[Files] (Filename, OriginalName, UserId)
        SELECT ''t_testdata_userShared1.jpg'', ''t_testdata_userShared1.jpg'', U.Id FROM ['+@dbSchema+'].[Users] U
        WHERE U.Username = ''t_testdata_CustomUser3''

        INSERT INTO ['+@dbSchema+'].[Files] (Filename, OriginalName, UserId)
        SELECT ''t_testdata_default.jpg'', ''t_testdata_default.jpg'', U.Id FROM ['+@dbSchema+'].[Users] U
        WHERE U.Username = ''Default''  
	             
        INSERT INTO ['+@dbSchema+'].[GroupShares] (PermissionId, GroupId, Filename)
        SELECT P.Id, G.Id, ''t_testdata_groupShared1.jpg'' FROM  ['+@dbSchema+'].[Permissions] P, ['+@dbSchema+'].[Groups] G
        WHERE P.Name = ''Full'' AND G.Name = ''t_testdata_CustomGroup''
        
        INSERT INTO ['+@dbSchema+'].[UserShares] (PermissionId, UserId, Filename)
        SELECT P.Id, U.Id, ''t_testdata_userShared1.jpg'' FROM  ['+@dbSchema+'].[Permissions] P, ['+@dbSchema+'].[Users] U
